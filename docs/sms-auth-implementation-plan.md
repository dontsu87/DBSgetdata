...
> [!CAUTION]
> **本書は 2026-08-01 時点で置き換え済みです（Gmail API 前提の旧計画）。**
> 実際の刷新後は認証コードが Office 365 のメールボックスに届く仕様となり、
> Power Automate → OneDrive 共有ファイル経由で受け渡す方式に変更されました。
> 現行の仕様は [`email-2fa-power-automate-spec.md`](./email-2fa-power-automate-spec.md) を参照してください。
> 本書はセッション管理方式の事前調査結果（現行システムは Cookie ではなく
> hidden パラメータ `SessionID` 依存）の記録として残しています。
