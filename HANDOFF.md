...
| 本番稼働 | **停止中**。`announcement.json` のメンテナンスモードで定期実行をスキップ中 |

最後に成功した本番取得は `output/車両情報_20260731_210130.csv`。以降データは更新されていない。
...
`announcement.json` の `maintenance.enabled` を `false` にすると5分ごとの自動実行が再開する。
**結合テストが通り、利用者が承認するまで絶対に変更しないこと。**
...
- 利用者の承認後にのみ `announcement.json` の `maintenance.enabled` を `false` にして定期実行を再開する
---
...
未確認なのは外部書き込みを伴うR2アップロードと、実タスクスケジューラによる連続運転。メンテナンス解除は未実施。
...
- メンテナンスモードは引き続き有効。解除していない
- CSV・ダッシュボード生成の結合は一時領域で検証済みだが、今回の実機確認では外部/生成物書き込みを無効化した
- R2アップロードと実タスクスケジューラによる連続運転は未確認
- 利用者の明示承認後にのみメンテナンスを解除して定期実行を再開する