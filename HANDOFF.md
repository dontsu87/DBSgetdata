...
## 更新（2026-09-11: ポート更新予約のローリング管理）

管理ポータル標準の最大2件の更新予約を、ポート別の週次レシピから照合・補充する
`src/port_booking_scheduler.py`を追加した。通常実行はGETだけの差分確認で、`--apply`を明示した場合だけ
空き枠へ新規予約をPOSTする。既存予約のPUT/DELETEは行わない。

- 複数レシピを定義し、複数ポートへ個別に割り当て可能
- 運用状態、公開設定、駐輪台数制限を実値または`inherit`で指定可能
- `inherit`項目の予約値と現在値が変わった場合、予約を更新せず競合としてSlack通知
- レシピ外予約、同時刻重複、2枠不足、登録直前の競合も書き込みせず停止
- POST応答を失った場合はGETで完全一致を確認し、二重登録を防止
- 多重起動ロック、結果JSON、同一通知の30分抑止を実装

実機プローブでは金沢「00.試験用ポート」で、`GET /api/ports/{id}/bookings`、
`POST /api/ports/{id}/bookings`と4項目の本文契約を確認した。現在値と同じ状態を
2026-09-12 23:45 JSTへ1件登録し、HTTP 200、GET完全一致1件、現在状態不変を確認した。
この予約は実行時刻まで2枠中1枠を使用する。

設定例は`config/port_booking_recipes.json.example`、テストは
`tests/test_port_booking_scheduler.py`。2026-09-11時点で全体テスト205件合格。

同日、金沢「81.学生のまち市民交流館」向けの仮レシピを、Git管理外の
`config/port_booking_recipes.json`へ作成した。通常週は日曜22:00に「一時休止中」、
火曜10:00に「運用中」とし、公開・駐輪台数制限有効・上限5台を明示する。
実ポータルの既存予約（2026-09-13 22:00、2026-09-15 10:00）とのGET照合は`in_sync`。
利用者がこの2件を削除した後、ポート本体と現在値が残り予約だけ0件であることをGET確認してから
`--apply`を実行し、同じ2件を再登録した。登録後のGETでは日時・運用状態・公開フラグ・
駐輪台数制限・上限5台が完全一致し、続く読み取り実行も`in_sync`。検証記録は
`output/port_booking_tests/81_gakusei_no_machi/20260911_recreation_verification.json`に保存した。
公式ルールの「月曜が休日なら直後の平日」および12月29日から1月3日は、この通常週レシピの
対象外であり、例外日対応を実装・検証してから定常運用へ移す。Windowsタスク登録は未実施。

予約状況のURL表示用に`booking-status/`を追加し、スケジューラが内部IDを除いた
`port_booking_status.json`を生成するようにした。`--publish-status`指定時だけ既存R2へ送る。
2026-09-12にR2へ公開し、GitHub Pages用ファイルと拡張プレビュー版 `202609120816` を
今回の対象差分として準備した。本番拡張チャンネルは変更していない。
...
3. 既存の認証済みブラウザセッションを使い、画面遷移・読み取り専用GET・ダウンロードだけを行う調査は、
   5分ごとの定期実行中でも実施してよい。認証情報、Cookie、Storageの値は採取・表示しない。
   新規ログイン、MFA、セッションファイルの保存・更新、または本番出力と競合する書き込みを伴う調査では、
   事前に `check_maintenance_mode()` が `True`（＝定期実行が止まっている）ことを確認する。
...
## 12. 利用履歴TSVの日次取得に向けた非破壊調査（2026-08-18）

### 調査範囲

- 保存済みの認証セッションを `requests.Session` へ復元し、管理ポータルへの読取・抽出要求だけを実施した。
- 認証情報、Cookie値、個別利用履歴の値はログへ出力していない。
- 実測対象は金沢エリア、確定済みの前日 `2026-08-17` の1日分。
- 調査中も5分タスクは継続稼働し、既存出力・セッションファイルは変更していない。

### 確定したAPIフロー

利用履歴はTSVを直接返す単発GETではなく、非同期抽出ジョブである。

1. `POST /api/invoices/personal/charge-settlement/usage-history/job`
   - JSON body: `areaId`, `searchPeriodFrom`, `searchPeriodTo`
   - 日付は `YYYYMMDD`。
   - 応答の `jobId` を保持する。
2. `GET /api/invoices/personal/charge-settlement/usage-history/job?jobId=...`
   - `status` が完了になるまで5秒間隔でポーリングする。
   - 実応答の状態値は小文字の `running` / `completed`。大小文字を正規化して判定すること。
   - 完了応答の `dataList` は日付ごとの `{date, areaCode, records}`。

旧定義の `/api/invoices/personal/charge-settlement/usage-history` を直接GETすると404になるため、
必ず `/job` のPOST＋GETフローを使う。

### 実測結果

- 金沢、`2026-08-17` は `dataList` 1グループ、2,449レコード。
- 抽出ジョブは約5秒で完了し、完了JSONは約2.4MB。
- エリアコードは `KNZ`、日付は `20260817` として返る。
- 画面側のZIP名は `usage_history_<areaCode>.zip`。
- ZIP内は日付ごとに `usage_history_<areaCode>_<YYYYMMDD>.tsv` を1ファイル生成する。
...
画面に並ぶ「料金オプション別利用回数」等は複数のダウンロード種別であり、利用履歴ファイルの列数ではない。

### TSV生成仕様

- UTF-8 BOM付き。
- タブ区切り、CRLF改行、末尾にもCRLFを付ける。
- タブ・改行・ダブルクォートを含む値はダブルクォートで囲み、値中のダブルクォートは二重化する。
- APIはJSONを返し、TSVとZIPはフロントエンド側で生成している。自動取得ではZIPを再現せず、
  `output/useage` へ日付単位のTSVを直接確定保存する設計が可能。

### 実装時の運用方針

- 実行時に `/api/areas` からエリアを取得し、IDをハードコードしない。
- 毎回「昨日」までを確定範囲とし、当日分は取得・確定しない。
- 既存5分タスク内の最初の成功スロット（現在の周期では5時頃）から日次処理を呼ぶ方式なら、
  別タスクとの競合を避けられる。既存の日次GBFS処理と同様に完了マーカーを持たせる。
- 一時ファイルへ書き、列・対象日・レコード構造を検証後に原子的に確定名へ置換する。
- 確定済みの日付ファイルは上書きせず、未取得日のみ補完する。
- セッション無効時は既存認証更新フローへ委ね、抽出ジョブ自体には認証処理を持ち込まない。

---

## 17. Power BI用の日次利用データ実装（2026-08-18）

- 対象は「利用履歴」と「料金オプション別アクティブユーザ数」の2種類だけ。
- 前日以前だけを確定対象とし、全エリアを日付・データセット単位のParquetへまとめる。
- 保存先は `output/useage/<dataset>/YYYY/MM/<dataset>_YYYYMMDD.parquet`。
- `.part` への書き込み、スキーマ・対象日・対象エリアの再検証、SHA-256計算後に原子的に確定する。
- SQLite台帳とParquetメタデータで確定・アップロード状態を管理し、確定ファイルを再取得・上書きしない。
- 同じ出力先への並行実行はOSのファイルロックで拒否する。
- 既存5分タスクでは05時以降の最初の車両取得成功後に実行し、昨日を優先、残枠で古い欠損を補完する。
- ローカル確定後のOneDrive失敗は `pending` とし、次スロットではAPIを再取得せずアップロードだけ再試行する。
- 専用送信先は `ONEDRIVE_USAGE_SHARED_LINK` / `ONEDRIVE_USAGE_PASSWORD`。既存のOneDrive設定へフォールバックしない。
- 日次Parquetのアップロードは、送信後に共有フォルダを再読込し、対象ファイル名を確認してから成功扱いにする。
- 専用設定と `DBS_USAGE_DAILY_ENABLED=true` が入るまでは自動処理を開始しない。
- 過去欠損の明示補完は `main.py --usage-backfill --usage-from YYYY-MM-DD --usage-to YYYY-MM-DD`。
- 実データ取得・実アップロードは未実施。外部通信をモックした関連テスト31件は成功。

## 18. 日次処理の本体タスク分離と自動過去補完の扱い（2026-08-18）

- 既存のWindowsタスク `DBS_Scraper_5Min` は変更せず、`main.py` が日次Parquet処理を
  `--usage-daily-worker` の独立子プロセスとして起動する。本体は子プロセスの完了を待たず、
  車両CSV後のR2アップロードへ進む。
- 子プロセスは既存のParquet排他ロック・完了マーカーを共有するため、5分ごとに起動要求が
  あっても実処理は1本だけになる。ログは `output/useage/_state/daily_usage_worker.log`。
- `DBS_USAGE_AUTO_BACKFILL_ENABLED` の既定値をfalseに変更。通常の日次処理は昨日分だけを扱い、
  過去分は `--usage-backfill` の明示実行に限定する。
- 2025-09-01のファイルは、旧実装の「最古の欠損日補完」が `DBS_USAGE_START_DATE` の既定値
  2025-09-01を選択して生成したもの。利用履歴0行、アクティブユーザー数67行で、8月ファイル内への
  日付混入ではない。既存ファイル・OneDrive上の送信済みファイルは削除していない。
- 実ホスト確認では、バックフィル完了後の15:14・15:19スロットが終了コード0となり、車両CSVも
  15:15・15:20に更新された。

---

## 19. AT異常系ステータスの細分化復元および強調表示改修（2026-08-27）

- 新ポータルAPI（`/api/vehicles`）の `attachmentAbnormalState` に基づき、一律「AT異常全般」に集約されていた車両状態を新ポータル公式の10種類の詳細ステータスへ細分化復元した。
- `src/new_portal_scraper.py` に `ATTACHMENT_ABNORMAL_LABELS` を定義し、`vehicleState == "ATTACHMENT_ABNORMAL"` の場合に `attachmentAbnormalState` を参照して「AT異常(AT通知受信なし)」「AT異常(電圧値閾値未満)」等の正式な日本語ラベルにマッピング（フォールバックは「AT異常全般」）。
- フロントエンドのデフォルト強調表示（`DEFAULT_HIGHLIGHT_STATUSES`）を `['AT異常(AT通知受信なし)', 'AT異常(電圧値閾値未満)', 'メンテナンス(手動)']` の3点に設定。
- 既存ブラウザキャッシュからのマイグレーション処理を実装（「AT異常全般」や旧名称「AT異常(電池なし)」を新ステータスへ自動移行）。
- `js/ui.js` の `alwaysVisibleStatuses` も上記3点に更新。
- `index.html` のバージョンを `ver.20260827113500` に更新。ブラウザのスクリプトキャッシュ保持を防ぐため、`style.css` および `js/*` の読み込みタグに `?v=20260827113500`（キャッシュバスター）を付与。
- `tests/e2e_current_portal_ui.py` の強調表示アサーションを新3点に合わせて更新。
- 回帰テスト175件すべてパス、Playwright E2Eテストおよび実API取得による細分化出力を検証済み。


---

## 20. 未施錠未返却車両の検知・表示改善（ポート再配置 ＆ 静止2時間判定）（2026-08-27）

### 背景と原因
- ドコモ・バイクシェア新管理ポータル側において、「利用中」車両に貸出ポート名が付与され続けるバグ（仕様）が継続しており、現場のポート（尾山神社前 NNI042、東金沢駅東口 KNZ0244 等）に未施錠放置されている車両が貸出元ポート名に紐づいたままになっていた。
- 位置不整合抽出処理（`preparePositionMismatchData`）において、「利用中」かつ不整合の車両が一律仮想ポート「ポート外（位置不整合）」に切り離され、座標がないためポートピンおよびサマリパネルの「🔑 未施錠未返却」カウントから除外されていた。
- さらに、地図上のポート外ドットマーカーでも、位置不整合モードOFFかつ利用中強調OFFの通常時は走行中ノイズ除外フィルターにより非表示になっていた。

### 改修内容
1. **実在ポート敷地内（100m以内）の位置不整合車両の最寄りポート再配置（`js/map.js`）**:
   - `preparePositionMismatchData` において、`findNearestPort` で100m以内の実在ポートが特定できた場合、仮想ポートではなくその最寄り実在ポートの `bikes` に再配属する。
   - `mismatch_source_port`（貸出元ポート名）を保持し、ポートカード上で「⚠️ポート位置不整合（元: ○○）」と表示。
   - 実在ポートに 🔑（未施錠アイコン）が点灯し、サマリパネルの「🔑 未施錠未返却」台数にも正常に加算される。
2. **バックエンド: 静止状態追跡機能（`src/stationary_tracker.py`, `src/exporter.py`）**:
   - `output/vehicle_stationary_state.json` を永続化し、5分毎に各「利用中」車両のGPS座標の推移を判定。
   - 移動が50m以内の場合は静止継続（`stationary_started_at` を維持し時間を積算）、50m超の移動を検知した場合は「移動中」とみなして静止タイマーを即時リセット。
   - CSVおよび `dashboard_data.json` に `静止開始日時`（`stationary_started_at`）と `静止継続時間(秒)`（`stationary_duration`）を出力。
3. **ポート外（100m超の路上等）の未施錠カギマーク表示（`js/map.js`, `style.css`）**:
   - 実在ポートから100m以上離れたポート外の「利用中」車両について、「静止判定開始から2時間（7,200秒）以上」経過した場合のみ路上乗り捨て未施錠として地図上に点灯。
   - ユーザー要望に基づき、ポートピンの 🔑 アイコン（16px）と明確に区別がつくよう、ポート外のドットマーカー上の 🔑 アイコンは少し小さめ（11px控えめ）に表示。
   - 移動中の通常利用車両は静止時間が短いため非表示（ノイズ除外）を維持。
4. **バージョンとキャッシュバスター更新**:
   - `index.html` のバージョンを `ver.20260827150000` に更新し、外部ファイルURLのクエリパラメータも更新。

### 検証
- `tests/test_stationary_tracker.py`（静止積算、50m超リセット、返却クリーンアップ）合格。
- `tests/test_frontend_filters.js`（最寄り実在ポートへの再配属ロジック）合格。
- 全pytest回帰テスト **177件成功**。
- Playwright E2Eテスト **全件成功**。
- 最新実データ（`KNZ0244`）において、武道館から「37.東金沢駅東口」への再配属と22.5時間未施錠の正常認識を検証済み。
