...
(function (global) {
  ...
  function buildPrintViewHtml(params) {
    ...
    for (var cIdx = 0; cIdx < corporations.length; cIdx++) {
      ...
      html.push('      <img src="' + SEAL_DATA_URI + '" alt="印" style="position: absolute; right: 8px; top: 38px; width: 68px; height: 68px; pointer-events: none; opacity: 0.88;">');
      html.push('    </div>');
      html.push('  </div>');
      ...
      html.push('    <div style="font-size: 12px; color: #666;">シェアサイクル まちのり</div>');
      html.push('  </div>');
      ...
      html.push('        <th style="width: 10%;">利用種別</th>');
      ...
      for (var u = 0; u < users; u++) {
        ...
        html.push('        <td style="text-align: center;">基本料</td>');
        html.push('        <td style="text-align: right;">' + formatMoney(basicUnit) + '</td>');
        html.push('        <td style="text-align: center;">-</td>');
        html.push('        <td style="text-align: center;">-</td>');
        html.push('        <td style="text-align: center;">-</td>');
        html.push('        <td style="text-align: center;">-</td>');
      ...
      for (var h = 0; h < hList.length; h++) {
        ...
        // 月額基本料レコードやすでに基本料金として計上したものはスキップ
        if (rec.invoiceItemName === '月額基本料' || (!usageObj.rentalPortName && !usageObj.rentalStartTs)) {
          continue;
        }

        var priceVal = getDetailPrice(rec);
        var itemKind = rec.invoiceItemName || rec.chargeName || '利用';
        var rPort = usageObj.rentalPortName || '-';
        var retPort = usageObj.returnPortName || '-';
        var rDate = formatDateTime(usageObj.rentalStartTs || usageObj.rentalDateTime);
        var retDate = formatDateTime(usageObj.returnTs || usageObj.returnDateTime);
        var uCode = usageObj.userUniqueCode || '-';

        html.push('      <tr>');
        html.push('        <td style="text-align: center;">' + rowNum++ + '</td>');
        html.push('        <td style="font-size: 11px;">' + escapeHtml(uCode) + '</td>');
        html.push('        <td style="text-align: center;">' + escapeHtml(itemKind) + '</td>');
        html.push('        <td style="text-align: right;">' + formatMoney(priceVal) + '</td>');
        html.push('        <td>' + escapeHtml(rPort) + '</td>');
        html.push('        <td style="font-size: 11px; text-align: center;">' + escapeHtml(rDate) + '</td>');
        html.push('        <td>' + escapeHtml(retPort) + '</td>');
        html.push('        <td style="font-size: 11px; text-align: center;">' + escapeHtml(retDate) + '</td>');
        html.push('      </tr>');
      }
  ...
  function showInvoiceModal() {
    ...
    submitBtn.addEventListener('click', function () {
      // ユーザーのクリックの瞬間（同期）にプレビュー用タブを先行確保
      // （※非同期完了後に window.open するとブラウザにポップアップブロックされるため）
      var previewWin = null;
      try {
      ...
      fetchServiceCompanyId()
        ...
        .then(function (corpDataList) {
          ...
          if (previewWin && !previewWin.closed) {
            ...
            // 万一ポップアップがブロックされていた場合は、モーダル内に直接開くボタンを提示
