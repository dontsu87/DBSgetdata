...
(function (global) {
  ...
  function getCorporateClientId() {
    if (typeof location === 'undefined') return '';
    var m = (location.pathname || '').match(/^\/corporate-clients\/([^/]+)\/histories/);
  ...
  function getCorporateHistoriesDates() {
    var dateFrom = '';
    var dateTo = '';
    if (typeof location !== 'undefined' && location.search) {
      dateFrom = getQueryParam(location.search, 'searchTargetDateFrom');
      dateTo = getQueryParam(location.search, 'searchTargetDateTo');
    }
    if (!dateFrom || !dateTo) {
      if (typeof document !== 'undefined') {
        var inputs = document.querySelectorAll('input.el-range-input');
        if (inputs.length >= 2) {
          var v0 = (inputs[0].value || '').trim();
          var v1 = (inputs[1].value || '').trim();
          if (v0) dateFrom = v0.replace(/\//g, '-');
          if (v1) dateTo = v1.replace(/\//g, '-');
        }
      }
    }
  ...
  function buildCorporateHistoriesTsv(items) {
    var lines = [CORPORATE_HISTORIES_HEADERS.join('\t')];
    for (var i = 0; i < items.length; i++) {
      var a = items[i] || {};
      var u = a.usage || {};

      var ridingCode = (u && u.ridingUniqueCode) ? u.ridingUniqueCode : (a.ridingId || '');
      var userCode = (u && u.userUniqueCode) ? u.userUniqueCode : '';
      var chargeName = a.chargeName != null ? a.chargeName : '';
      var itemName = a.invoiceItemName != null ? a.invoiceItemName : '';
      var taxPrice = a.taxIncludedPrice != null ? a.taxIncludedPrice : '';
      var taxTotal = a.taxIncludedTotalPrice != null ? a.taxIncludedTotalPrice : '';
      var offsetPrice = a.rentalOffsetPrice != null ? a.rentalOffsetPrice : '';
      var extPrice = a.rentalExtensionPrice != null ? a.rentalExtensionPrice : '';
      var basicCharge = a.basicCharge != null ? a.basicCharge : '';
      var addPrice = a.additionalPrice != null ? a.additionalPrice : '';
      var updateTs = a.updateTs != null ? a.updateTs : '';
      var updateReason = a.updatedReason != null ? a.updatedReason : '';
      var paymentType = a.paymentType != null ? a.paymentType : '';
      var rentalPort = (u && u.rentalPortName) ? u.rentalPortName : '';
      var rentalTs = (u && u.rentalStartTs) ? u.rentalStartTs : '';
      var returnPort = (u && u.returnPortName) ? u.returnPortName : '';
      var returnTs = (u && u.returnTs) ? u.returnTs : '';
      var vehCode = (u && u.vehicleUniqueCode) ? u.vehicleUniqueCode : '';
      var returnKind = (u && u.returnKind) ? u.returnKind : '';
      var areaName = (u && u.areaName) ? u.areaName : '';

      var row = [
        ridingCode, userCode, chargeName, itemName,
        taxPrice, taxTotal, offsetPrice, extPrice,
        basicCharge, addPrice, updateTs, updateReason,
        paymentType, rentalPort, rentalTs, returnPort,
        returnTs, vehCode, returnKind, areaName
      ];

      var formattedRow = [];
      for (var j = 0; j < row.length; j++) {
        formattedRow.push(formatTsvCell(row[j]));
      }
  ...
  function fetchCorporateHistoriesAll(corporateId, dateFrom, dateTo) {
    var allItems = [];
    var pageSize = 500;
    var fetchFn = typeof fetch !== 'undefined' ? fetch : (typeof window !== 'undefined' && window.fetch ? window.fetch : null);
    if (!fetchFn) return Promise.reject(new Error('fetch unavailable'));

    function fetchPage(page) {
      var url = '/api/corporate-clients/' + encodeURIComponent(corporateId) + '/histories' +
                '?searchTargetDateFrom=' + encodeURIComponent(dateFrom) +
                '&searchTargetDateTo=' + encodeURIComponent(dateTo) +
                '&page=' + page +
                '&pageSize=' + pageSize;
      ...
      return fetchFn(url, { credentials: 'same-origin' })
        ...
        .then(function (data) {
          ...
          var total = pagination && typeof pagination.total === 'number' ? pagination.total : allItems.length;
  ...
  function hookCorporateHistoriesExport() {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      // ファイル出力ボタン: キャプチャフェーズで先回りし、全件TSVを安全に出力
      if (text.indexOf('ファイル出力') !== -1 && !btn._dbsextExportHooked) {
        ...
        btn.addEventListener('click', function (e) {
          ...
          var corporateId = getCorporateClientId();
          if (!corporateId) {
            log('法人契約IDが特定できません', true);
            return;
          }

          var dates = getCorporateHistoriesDates();
          if (!dates.dateFrom || !dates.dateTo) {
            log('検索期間が指定されていません', true);
            return;
          }

          var targetBtn = e ? e.currentTarget : btn;
          if (targetBtn && targetBtn.style) {
            targetBtn.style.opacity = '0.6';
            targetBtn.style.pointerEvents = 'none';
          }

          fetchCorporateHistoriesAll(corporateId, dates.dateFrom, dates.dateTo)
            .then(function (items) {
              if (targetBtn && targetBtn.style) {
    ...
    _state: function () {
      return {
        ...
        buildCorporateHistoriesTsv: buildCorporateHistoriesTsv,
        formatTsvCell: formatTsvCell,
        fetchCorporateHistoriesAll: fetchCorporateHistoriesAll,
