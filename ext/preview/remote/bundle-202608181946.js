 ...
 *   - リクエスト本文キーは固定ホワイトリスト 5種のみ（実行時検証、違反時は即座に中断）。
 *   - 取得結果は完全一致した { userKind, userId } のみ保持し、氏名・電話・メール等は一切保持・ログ出力しない。
 *   - 同時実行上限（最大3並列）、1画面自動上限（最大30件）、session cache（TTL 2分、negative cache対応）。
...
(function (global) {
  ...
  var cacheMap = {};
  var CACHE_TTL_MS = 120000; // 2分 (session cache)
   ...
   */
  function fetchUserByKind(userUniqueCode, userKind, signal) {
    if (!isValidUserUniqueCode(userUniqueCode) || !USER_KINDS[userKind]) {
    ...
    return fetch('/api/users', {
      ...
      .then(function (data) {
        ...
        for (var i = 0; i < items.length; i++) {
          var item = items[i];
          if (item && item.userUniqueCode === userUniqueCode && item.userKind === userKind && isValidUserKind(item.userKind)) {
  ...
  function fetchSummary(userUniqueCode, options) {
    ...
    if (!opts.force) {
      ...
      if (!pageRequestedCodes[userUniqueCode]) {
        if (requestedCount >= MAX_AUTO_FETCH_PER_PAGE) {
    ...
    var promise = new Promise(function (resolve) {
      ...
      function tryNextKind() {
        ...
        enqueueFetch(function () {
          ...
          return fetchUserByKind(userUniqueCode, kind, signal);
  ...
  /**
   * 画面内に現れた userUniqueCode 群を一括照会する (1画面上限30件)。
    ...
    if (isEligible) {
      ...
      for (var t = 0; t < tables.length; t++) {
        ...
        if (D.userSummary && typeof D.userSummary.requestSummaries === 'function') {
          ...
          D.userSummary.requestSummaries(codesToQuery, function (resolvedCode, summary) {
            for (var k = 0; k < cellBindingMap.length; k++) {
              if (cellBindingMap[k].code === resolvedCode) {
                applySummaryToBinding(cellBindingMap[k], summary);
              }
            }
          });
        }
