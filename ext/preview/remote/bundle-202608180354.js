...
(function (global) {
  ...
  /**
   * 単一テーブルの列幅・操作列表示の適用（静的CSS方式・表全体幅設定なし・真のno-op）
   */
    ...
    if (canApplyStaticWidths) {
      ...
      for (var c = 0; c < colCount; c++) {
        ...
        if (targetWidth !== null && targetWidth > 0) {
          var wStr = targetWidth + 'px';
          if (headerCols[c]) {
            backupOriginalStyle(headerCols[c], 'width');
            backupOriginalStyle(headerCols[c], 'min-width');
            backupOriginalStyle(headerCols[c], 'max-width');
            headerCols[c].style.setProperty('width', wStr, 'important');
            headerCols[c].style.setProperty('min-width', wStr, 'important');
            headerCols[c].style.setProperty('max-width', wStr, 'important');
          }
          if (bodyCols[c]) {
            backupOriginalStyle(bodyCols[c], 'width');
            backupOriginalStyle(bodyCols[c], 'min-width');
            backupOriginalStyle(bodyCols[c], 'max-width');
            bodyCols[c].style.setProperty('width', wStr, 'important');
            bodyCols[c].style.setProperty('min-width', wStr, 'important');
            bodyCols[c].style.setProperty('max-width', wStr, 'important');
          }
          if (th) {
            backupOriginalStyle(th, 'width');
            backupOriginalStyle(th, 'min-width');
            backupOriginalStyle(th, 'max-width');
            th.style.setProperty('width', wStr, 'important');
            th.style.setProperty('min-width', wStr, 'important');
            th.style.setProperty('max-width', wStr, 'important');
          }
          for (var brIdx = 0; brIdx < allBodyRows.length; brIdx++) {
            var bodyTd = allBodyRows[brIdx].children[c];
            if (bodyTd) {
              backupOriginalStyle(bodyTd, 'width');
              backupOriginalStyle(bodyTd, 'min-width');
              backupOriginalStyle(bodyTd, 'max-width');
              bodyTd.style.setProperty('width', wStr, 'important');
              bodyTd.style.setProperty('min-width', wStr, 'important');
              bodyTd.style.setProperty('max-width', wStr, 'important');
            }
          }
        }
      }
      ...
      for (var rIdx = 0; rIdx < colCount; rIdx++) {
        ...
        for (var clrIdx = 0; clrIdx < allBodyRows.length; clrIdx++) {
          var clrTd = allBodyRows[clrIdx].children[rIdx];
          if (clrTd) {
            restoreOriginalStyle(clrTd, 'width');
            restoreOriginalStyle(clrTd, 'min-width');
            restoreOriginalStyle(clrTd, 'max-width');
          }
        }
      }
    }
