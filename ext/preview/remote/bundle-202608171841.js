...
(function (global) {
  ...
  var portSortHooked = false;

  function sortPortDropdownItems() {
    var portSortKey = D.tableTools && D.tableTools.portSortKey;
    if (!portSortKey) return;

    var select = findSelectByLabel('ポート');
    if (!select) return;

    // ドロップダウンは teleport で body 直下に描画される。最後の visible なものを取る
    var dropdown = null;
  ...
  function hookPortSelect() {
    if (portSortHooked || typeof document === 'undefined') return;
    ...
    trigger.addEventListener('click', function () {
      setTimeout(sortPortDropdownItems, 50);
    }, true);
  }
...
(function (global) {
  ...
  function loadAutoFitState(screen) {
    ...
    return true; // 既定は「ON」（autoFitCols = true）
  }
