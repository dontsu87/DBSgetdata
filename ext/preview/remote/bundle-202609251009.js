...
(function (global) {
  ...
  function fitVehicleTable(table) {
    ...
    var tableHeight = Math.max(portalCap, available);
    ...
    // 外枠と行領域に別々の上限を掛けると、縦長画面では表の下に空白が残る。
    var bodyHeight = Math.max(0, tableHeight - header.getBoundingClientRect().height - barHeight);
  ...
  D.tableWrap = {
    ...
    apply: function () {
      ...
      for (var t = 0; t < tables.length; t++) {
        ...
        // 帯の高さはクラス追加後に確定する。確定前に測ると行領域がその分はみ出す。
