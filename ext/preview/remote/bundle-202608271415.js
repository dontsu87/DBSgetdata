...
(function (global) {
  ...
  function findCorporateHistoriesTable() {
    if (typeof document === 'undefined') return null;

    // 1. まずヘッダーまたは内部に「乗車ID」または「ユーザ識別番号」を持つ el-table を探す
    var elTables = document.querySelectorAll('.el-table');
    for (var i = 0; i < elTables.length; i++) {
      var elTable = elTables[i];
      var text = elTable.textContent || '';
      if (text.indexOf('乗車ID') !== -1 || text.indexOf('ユーザ識別番号') !== -1) {
        var body = elTable.querySelector('.el-table__body-wrapper table.el-table__body, table.el-table__body, table');
        if (body) return body;
      }
    }

    // 2. カレンダー除外 (.el-date-table, .el-picker-panel を明示的に排除)
    var allTables = document.querySelectorAll('table.el-table__body, .el-table__body-wrapper table, table');
    for (var j = 0; j < allTables.length; j++) {
      var tb = allTables[j];
      if (tb.classList && tb.classList.contains('el-date-table')) continue;
      if (tb.closest && tb.closest('.el-picker-panel, .el-date-picker, .el-date-range-picker')) continue;
      return tb;
    }
  ...
  function exportCorporateHistoriesTable() {
    if (typeof document === 'undefined') return false;
    var table = findCorporateHistoriesTable();
    if (!table) {
    ...
    var rows = [];
    var trs = table.querySelectorAll('tbody tr.el-table__row, tr.el-table__row, tbody tr, tr');
    for (var i = 0; i < trs.length; i++) {
      var tr = trs[i];
      if (tr.classList && (tr.classList.contains('el-table__empty-block') || tr.classList.contains('el-table__empty-text'))) {
        continue;
      }
      var tds = tr.querySelectorAll('td.el-table__cell, td');
      if (!tds || tds.length === 0) continue;

      // カレンダー（7列）などの誤混入を完全に防止（利用履歴表は20列）
  ...
  function hookCorporateHistoriesExport() {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      if (text === 'ファイル出力' || text.indexOf('ファイル出力') !== -1) {
        exportBtn = b;
        break;
      }
    }
    if (!exportBtn || exportBtn._dbsextExportHooked) return;
    exportBtn._dbsextExportHooked = true;
    ...
    exportBtn.addEventListener('click', function (e) {
      ...
      try {
        exportCorporateHistoriesTable();
      } catch (err) {
        log('法人契約利用履歴ファイル出力エラー: ' + (err && err.message ? err.message : err), true);
      }
    }, true);
  }
  ...
  D.uiTweaks = {
    ...
    _state: function () {
      return {
        ...
        exportCorporateHistoriesTable: exportCorporateHistoriesTable,
