...
(function (global) {
  ...
  function shouldAutoCollapseCondition(condPanel) {
    ...
    // 車両情報はまず1000件表示へ遷移する。遷移前に畳むと、その1回を消費した
    // 状態のまま1000件側で開いてしまうため、page-size=1000到達後を初回とする。
    if (typeof location !== 'undefined' && /^\/vehicles\/?$/.test(location.pathname || '')) {
      var pageSizeMatch = /[?&]page-size=(\d+)/.exec(location.search || '');
      if (!pageSizeMatch || parseInt(pageSizeMatch[1], 10) < 1000) return false;
