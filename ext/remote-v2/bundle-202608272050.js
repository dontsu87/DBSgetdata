...
(function (global) {
  ...
  function renderUserIdCell(uid) {
    ...
    var len = uid.length;
    if (len >= 25) {
      // 列幅14.5%（約95px）に拡張されたため、フォント8.2px / scaleX(0.70)で大きく明瞭に表示
      return '<div style="display: inline-block; font-size: 8.2px; font-family: Arial, sans-serif; letter-spacing: -0.2px; transform: scaleX(0.70); transform-origin: left center; white-space: nowrap;">' + escapeHtml(uid) + '</div>';
    }
    if (len >= 18) {
      return '<div style="display: inline-block; font-size: 8.8px; font-family: Arial, sans-serif; letter-spacing: -0.1px; transform: scaleX(0.85); transform-origin: left center; white-space: nowrap;">' + escapeHtml(uid) + '</div>';
    }
    if (len >= 12) {
  ...
  function buildPrintViewHtml(params) {
    ...
    for (var cIdx = 0; cIdx < corporations.length; cIdx++) {
      ...
      for (var pIdx = 0; pIdx < totalPages; pIdx++) {
        ...
        html.push('    <thead>');
        html.push('      <tr>');
        html.push('        <th style="width: 4%;">NO.</th>');
        html.push('        <th style="width: 14.5%;">ユーザーID</th>');
        html.push('        <th style="width: 6.5%;">種別</th>');
        html.push('        <th style="width: 7%;">料金(税抜)</th>');
        html.push('        <th style="width: 19.5%;">利用開始ポート</th>');
        html.push('        <th style="width: 14%;">利用開始日時</th>');
        html.push('        <th style="width: 19.5%;">返却終了ポート</th>');
        html.push('        <th style="width: 14%;">返却終了日時</th>');
        html.push('      </tr>');
        html.push('    </thead>');
        html.push('    <tbody>');
        ...
        for (var r = 0; r < pageRows.length; r++) {
          ...
          html.push('        <td style="font-size: 8.3px; letter-spacing: -0.3px; text-align: center; padding: 4px 2px;">' + escapeHtml(rowItem.rDate) + '</td>');
          html.push('        <td title="' + escapeHtml(rowItem.retPort) + '">' + renderPortCell(rowItem.retPort) + '</td>');
          html.push('        <td style="font-size: 8.3px; letter-spacing: -0.3px; text-align: center; padding: 4px 2px;">' + escapeHtml(rowItem.retDate) + '</td>');
          html.push('      </tr>');
        }
