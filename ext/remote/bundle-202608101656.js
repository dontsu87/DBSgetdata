...
(function (global) {
  ...
  var ATTR_KIND = {
    ...
    'data-dbsext-vehicle-problems': 'own',
    'data-dbsext-vp-table': 'own',
  ...
  D.core = {
    ...
    boot: function () {
      ...
      var moduleOrder = [
        ...
        { name: 'vehicleProblems', target: function () { return D.vehicleProblems; } },
      ...
      D.core.onContentChange(function () {
        ...
        if (D.vehicleProblems && typeof D.vehicleProblems.apply === 'function') {
          runSuppressed('vehicleProblems.apply 再適用', function () { D.vehicleProblems.apply(); });
        }
  ...
  D.skin = {
    ...
    apply: function () {
      ...
      var css = [
        ...
        '  border: 1px solid ' + accent + ' !important;',
        '}',
        '[data-dbsext-vp-table] {',
        '  width: 100% !important;',
        '  border-collapse: collapse !important;',
        '  font-size: 13px !important;',
        '  background: #ffffff !important;',
        '}',
        '[data-dbsext-vp-table] th {',
        '  position: sticky !important;',
        '  top: 0 !important;',
        '  z-index: 2 !important;',
        ...
        '  white-space: nowrap !important;',
        '}',
        '[data-dbsext-vp-table] .dbsext-vp-sort {',
        '  display: block !important;',
        '  width: 100% !important;',
        '  border: none !important;',
        ...
        '  padding: 0 !important;',
        '}',
        '[data-dbsext-vp-table] .dbsext-vp-filter {',
        '  display: block !important;',
        '  width: 100% !important;',
        ...
        '  color: #ffffff !important;',
        '}',
        '[data-dbsext-vp-table] .dbsext-vp-filter::placeholder {',
        '  color: rgba(255,255,255,0.6) !important;',
        '}',
        '[data-dbsext-vp-table] td {',
        '  padding: 4px 8px !important;',
        '  border-bottom: 1px solid #e8e8e8 !important;',
        '  white-space: nowrap !important;',
        '}',
        '[data-dbsext-vp-table] tr:nth-child(even) td {',
        '  background: #f8f9fa !important;',
        '}',
        '[data-dbsext-vp-table] tr:hover td {',
        '  background: #e8f0fe !important;',
        '}',
        '[data-dbsext-vp-table] .dbsext-vp-empty {',
        '  text-align: center !important;',
        '  color: #999 !important;',
        '  padding: 24px !important;',
        '}',
        '[data-dbsext-vp-table] .dbsext-vp-link {',
        '  color: ' + accent + ' !important;',
        '  text-decoration: none !important;',
        '}',
        '[data-dbsext-vp-table] .dbsext-vp-link:hover {',
        '  text-decoration: underline !important;',
        '}'
      ].join('\n');
...
(function (global) {
  ...
  function renderTable(panel, rows) {
    ...
    var columns = [
      ...
      { label: 'ユーザ識別番号',  value: function (r) { return r.userUniqueCode; }, isLink: true, linkHref: function (r) { return '/users/' + (r.userUniqueCode || ''); } },
    ...
    table.className = 'dbsext-vp-table';
    ...
    function renderVisibleRows() {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        for (var f = 0; f < columns.length; f++) {
          var needle = String(filters[f].value || '').toLocaleLowerCase();
          if (needle && String(columns[f].value(rows[i])).toLocaleLowerCase().indexOf(needle) === -1) {
            matches = false;
            break;
          }
        }
        if (matches) indexed.push({ item: rows[i], originalIndex: i });
      }
      ...
      if (indexed.length === 0) {
        ...
        var emptyCell = document.createElement('td');
        emptyCell.setAttribute('colspan', String(columns.length));
        emptyCell.className = 'dbsext-vp-empty';
        emptyCell.textContent = '絞り込み条件に一致する申告はありません。';
        emptyRow.appendChild(emptyCell);
        tbody.appendChild(emptyRow);
        return;
      }
      ...
      for (var r = 0; r < indexed.length; r++) {
        ...
        for (var c = 0; c < columns.length; c++) {
          var td = document.createElement('td');
          if (columns[c].isLink && columns[c].linkHref) {
            var a = document.createElement('a');
            a.setAttribute('href', columns[c].linkHref(indexed[r].item));
            a.textContent = columns[c].value(indexed[r].item);
            a.className = 'dbsext-vp-link';
            td.appendChild(a);
          } else {
            td.textContent = columns[c].value(indexed[r].item);
          }
          tr.appendChild(td);
        }
        tbody.appendChild(tr);
    ...
    for (var h = 0; h < columns.length; h++) {
      (function (columnIndex) {
        ...
        var sortButton = document.createElement('button');
        sortButton.setAttribute('type', 'button');
        sortButton.className = 'dbsext-vp-sort';
        sortButton.setAttribute('title', 'クリックで並べ替え');
        sortButton.addEventListener('click', function () {
          if (sortColumn === columnIndex) {
            sortDirection = sortDirection * -1;
        ...
        var filterInput = document.createElement('input');
        filterInput.setAttribute('type', 'text');
        filterInput.className = 'dbsext-vp-filter';
        filterInput.setAttribute('placeholder', '絞り込み');
        filterInput.setAttribute('aria-label', columns[columnIndex].label + 'で絞り込み');
        filterInput.addEventListener('input', function () {
          renderVisibleRows();
