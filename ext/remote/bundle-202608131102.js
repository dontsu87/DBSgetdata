...
(function (global) {
  ...
  // 正しい`beaconIds`の値を復元する手段がこの機能には無い（手動PUT実測はキーの
  // 存在だけを記録し値の形式までは記録していない）ため、`fetchPortHasBeacons()`
  // （`GET /api/ports/{id}/beacons`、読み取り専用）で対象ポートにビーコンが
  // 1件でも紐づいているかを直前に確認し、紐づきが無いと確認できた場合だけ`[]`を補う。
  // 紐づきがある、または確認自体に失敗した場合は**推測でPUTせず、その行を失敗扱いにする**
  ...
  /**
   * 対象ポートにビーコンが1件でも紐づいているかを確認する（読み取り専用）。
   ...
   * 上記`REQUIRED_RECORD_KEYS`のコメント参照）。よって`beaconIds`を安全にPUTするには
   * この別APIで直接確認するしかない。取得失敗時は例外を投げ、呼び出し側で
   * 「確認できない」＝安全側で失敗扱いにする判断に使う。
   */
  function fetchPortHasBeacons(portId) {
  ...
  /**
   * `hasBeacons`: 対象ポートに`fetchPortHasBeacons()`で確認した「ビーコンが1件でも
   * 紐づいているか」。`true`/確認不能(`null`)の場合は`beaconIds`を推測で埋めず例外を投げる
   * （安全側優先。呼び出し側`processIndex`はこれをキャッチし、その行を失敗扱いにしてPUTしない）。
   * `record`に既に`beaconIds`が配列で含まれる場合はこの判定自体を通らない
   * （そのまま保持されるため呼ばなくてよい。`isRunStillValid`直後の分岐は`processIndex`側）。
   */
  function buildUpdatedRecord(record, serviceValue, publishValue, hasBeacons) {
    ...
    // つまりキー欠落を無条件に「ビーコン無し」とみなして`[]`を補う従来の実装は、
    // ビーコンが紐づくポートの紐付けを消していた。正しい値を復元する手段がこの機能には
    // 無いため、`hasBeacons`が`false`（`fetchPortHasBeacons()`で紐づき無しと直接確認できた）
    // ときだけ`[]`を補う。それ以外（紐づきあり、または確認不能）は推測で書き込まず、
    // この行自体を失敗させる。
    if (!Array.isArray(updated.beaconIds)) {
      if (hasBeacons === false) {
        updated.beaconIds = [];
      } else {
        throw new Error('ビーコン紐付けの有無を安全に確認できないため書き込みを見送りました');
      }
    }
    ...
    function processIndex(index) {
      ...
      fetchPortDetail(portId)
        .then(function (record) {
          ...
          // 追加の確認（`fetchPortHasBeacons`）は不要。無い場合だけ、ビーコン紐付けを
          // 誤って消さないよう直前に確認する（読み取り専用の追加GETが1回増える）
          if (Array.isArray(record.beaconIds)) {
            var updatedNoCheck = buildUpdatedRecord(record, serviceValue, publishValue, null);
            return putPortDetail(portId, updatedNoCheck);
          }
          return fetchPortHasBeacons(portId).catch(function () { return null; }).then(function (hasBeacons) {
            if (!isRunStillValid()) throw new Error('画面またはエリアが変わったため中断');
            var updated = buildUpdatedRecord(record, serviceValue, publishValue, hasBeacons);
